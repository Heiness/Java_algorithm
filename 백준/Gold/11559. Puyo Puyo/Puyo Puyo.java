...
public class Main {
	...
	static boolean[][] visited = new boolean[12][6], visitedCopy = new boolean[12][6];
	static boolean g;
	...
	static int bfs(Point p) {
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (0 <= nx && nx < 6 && 0 <= ny && ny < 12 && arr[ny][nx] == cur.c && visited[ny][nx] == false) { // 같은 색깔이고 방문안한 경우만 이동
					visited[ny][nx] = true;
